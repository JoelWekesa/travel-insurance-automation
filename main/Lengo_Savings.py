...
#     try:
#         emoji = "[OK]" if "Passed" in message or "Summary: All Passed" in message else "[X]"
...
#                     freq_select.click(timeout=3000, force=True)
#                     page.wait_for_timeout(500)
...
#             # Aggressive Continue button clicking - try multiple times
#             for attempt in range(5):
#                 try:
#                     continues = page.get_by_role("button", name="Continue")
#                     if continues.count() > 0:
#                         for idx in range(continues.count()):
#                             try:
#                                 btn = continues.nth(idx)
#                                 if btn.is_visible():
...
#                                     page.wait_for_timeout(2000)
#                                     page.wait_for_load_state("networkidle")
#                                     print(f"  Clicked Continue button {idx+1}")
#                                     break
#                             except Exception as e:
#                                 print(f"  Continue button {idx+1} failed: {e}")
#                                 continue
#                     else:
#                         break
#                 except Exception as e:
...
#         # Debug: Take screenshot before Step 3
#         page.screenshot(path=get_screenshot_path(plan_name, "2_after_continue.png"), full_page=True)
#         page.wait_for_timeout(2000)
...
#         # Wait longer and check if we're actually on Step 3
#         page.wait_for_timeout(3000)
#         page.screenshot(path=get_screenshot_path(plan_name, "3_before_fields.png"), full_page=True)
        
#         # Try to detect if Step 3 loaded by checking for any form fields
#         step3_loaded = False
#         try:
#             # Check if any Step 3 field exists
#             if page.locator("omk-select").count() > 0 or page.get_by_role("textbox").count() > 3:
#                 step3_loaded = True
#                 print(f"  Step 3 fields detected")
#         except:
#             pass
            
#         if not step3_loaded:
#             print(f"  WARNING: Step 3 fields not found, attempting to force navigation")
#             # Try clicking any visible Continue buttons again
#             try:
#                 continues = page.get_by_role("button", name="Continue")
#                 for i in range(continues.count()):
#                     try:
#                         btn = continues.nth(i)
#                         if btn.is_visible():
...
#         # Click Continue buttons with better handling
#         page.wait_for_timeout(1000)
#         continue_buttons = page.get_by_role("button", name="Continue")
#         if continue_buttons.count() > 0:
#             continue_buttons.first.click(force=True)
#             page.wait_for_timeout(1000)
#             page.wait_for_load_state("networkidle")
            
#             # Click second Continue if exists
#             if continue_buttons.count() > 0:
#                 continue_buttons.first.click(force=True)
#                 page.wait_for_timeout(1000)
#                 page.wait_for_load_state("networkidle")
...
#         # Wait for page to load and look for Add Beneficiary button
#         page.wait_for_timeout(2000)
#         page.screenshot(path=get_screenshot_path(plan_name, "4_before_beneficiary.png"), full_page=True)
        
#         add_ben_btn = page.get_by_role("button", name="Add Beneficiary")
#         add_ben_btn.wait_for(state="visible", timeout=15000)
#         add_ben_btn.click(force=True)
#         page.wait_for_timeout(1000)
...
#         try:
#             page.locator("input[type='file']").nth(0).set_input_files(id_doc)
#         except Exception as e:
#             print("Failed ID upload:", e)
#         page.wait_for_timeout(500)
        
#         try:
#             page.locator("input[type='file']").nth(1).set_input_files(kra_doc)
#         except Exception as e:
#             print("Failed KRA upload:", e)
#         page.wait_for_timeout(500)
...
#         status_slack_icon = "[OK] Passed" if res['status'] == 'Passed' else "[X] Failed"
...
#                 print(f"  [OK] {step}")
#                 summary_text += f"> [OK] {step}\n"
#             elif step == failed_step:
#                 print(f"  [X] {step} (Failed)")
#                 summary_text += f"> [X] {step} (FAILED)\n"
#             else:
#                 print(f"  [SKIP] {step} (Skipped)")
#                 summary_text += f"> [SKIP] {step} (SKIPPED)\n"
